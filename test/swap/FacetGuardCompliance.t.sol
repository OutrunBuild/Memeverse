...
contract FacetGuardComplianceTest is Test, HookStorageHelper {
    ...
    function test_FacetGuardInheritanceAndOnlyViaRouterLiteral() external view {
        ...
        for (uint256 i = 0; i < facets.length; ++i) {
            ...
            assertTrue(
                _contains(content, "DirectFacetCallForbidden") || _contains(vm.readFile("src/swap/FacetGuard.sol"), "DirectFacetCallForbidden"),
    ...
    function test_DirectCallToEachFacetReverts() external {
        ...
        _assertDirectReverts(
            hook.dynamicFeeFacet(),
            abi.encodeCall(IDynamicFeeFacet.prepareSwapFee, (_dummyPrepare()))
        );
        // SettlementFacet.executeSettlementLogic
        _assertDirectReverts(
            hook.settlementFacet(),
            abi.encodeCall(ISettlementFacet.executeSettlementLogic, (_dummySettlement()))
        );
        // SettlementFacet.settlementUnlockCallback also guarded (plus inner msg.sender == poolManager)
        _assertDirectReverts(
            hook.settlementFacet(),
            abi.encodeCall(ISettlementFacet.settlementUnlockCallback, (_dummyCallback()))
        );
    }
    ...
    function _dummyKey() internal pure returns (PoolKey memory) {
        return PoolKey({currency0: Currency.wrap(address(0)), currency1: Currency.wrap(address(0)), fee: 0x800000, tickSpacing: 200, hooks: IHooks(address(0))});
    }

    function _dummySwapParams() internal pure returns (SwapParams memory) {
        return SwapParams({zeroForOne: true, amountSpecified: -100 ether, sqrtPriceLimitX96: TickMath.MIN_SQRT_PRICE + 1});
    }

    function _dummyPrepare() internal view returns (IDynamicFeeFacet.PrepareSwapFeeParams memory) {
        return IDynamicFeeFacet.PrepareSwapFeeParams({poolId: PoolId.wrap(bytes32(0)), zeroForOne: true, amountSpecified: -100 ether, trader: address(this), preSqrtPriceX96: 0, liquidity: 0, protocolFeeOnInput: false, sqrtPriceLimitX96: 0});
    }

    function _dummySettlement() internal view returns (IMemeverseUniswapHook.PreorderSettlementParams memory) {
        return IMemeverseUniswapHook.PreorderSettlementParams({key: _dummyKey(), params: _dummySwapParams(), recipient: address(this)});
    }

    function _dummyCallback() internal view returns (ISettlementFacet.SettlementCallbackData memory) {
        return ISettlementFacet.SettlementCallbackData({recipient: address(this), treasury: address(this), key: _dummyKey(), swapParams: _dummySwapParams(), protocolFeeOnInput: false});
    }
    ...
    function _countOccurrences(string memory haystack, string memory needle) internal pure returns (uint256 count) {
        ...
        for (uint256 i = 0; i <= h.length - n.length; ++i) {
            bool isMatch = true;
            for (uint256 j = 0; j < n.length; ++j) if (h[i + j] != n[j]) { isMatch = false; break; }
            if (isMatch) { count++; i += n.length - 1; }
        }
    }
}
