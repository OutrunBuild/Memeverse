...
- 约束：固定 24 小时保护窗口仅是正常 `Locked -> Unlocked` 路径且 `launcher` binding 指向真实 Launcher proxy 时写入 pool-level `publicSwapResumeTime` 的产品语义；`Stage.Unlocked + hook` 按该 pool-level resume time 阻断公开 swap，赎回路径与公开 swap 可用性由不同模块分离控制。Hook owner 虽无直接 resume-time setter，仍可 retarget launcher 至受控地址；该地址可覆写既有非零 ERC-20 池的 resume time（`0` 清除阻断，过去/当前时间解除或缩短，未来时间延长或阻断），属于已声明的同一 Hook owner trust boundary，非无权限外部调用者能力。`[代码已证]`
- 活性约束：若 `launcher` binding 被 retarget 至不同于真实 Launcher proxy 的地址，真实 Launcher 在保护时间写入处不满足 `onlyLauncher`，正常 `Locked -> Unlocked` 以 `Unauthorized` 整笔回滚；调用 `changeStage` 前必须恢复真实 Launcher binding。`[代码已证]`
