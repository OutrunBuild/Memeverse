...
- LP token 转账（`transfer` / `transferFrom`）先经 `UniswapLP.sol::_update` 对 `from` / `to` 调用 `MemeverseUniswapHook.sol::updateUserSnapshot(poolId, ...)`，结晶 fee 快照：将可 claim fee 累入 `pendingFee0` / `pendingFee1` 并推进 offset（`SwapFacet.sol::updateUserSnapshotLogic`）`[代码已证]`。
  ...
  - add-liquidity 的快照→mint 窗口持 lifecycle lock：`MemeverseUniswapHook.sol::_addLiquidityCore` 在接收人 fee 快照之前 acquire、在 LP mint 与 `cachedLpTotalSupply` 更新之后 release，保证快照 offset 与 mint 时点 feePerShare 一致；remove-liquidity 的快照与 burn 先于 take 窗口、窗口后无份额发行，无需持锁。
