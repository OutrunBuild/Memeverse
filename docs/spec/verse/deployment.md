...
- 部署 runbook 注意（C1 write-once 固有代价，SECR-001/003）：广播 hook proxy 部署（`initialize` 绑定 `launcher_`）前，必须确认 `MEMEVERSE_LAUNCHER` env var 解析为预期的**已部署** launcher proxy——部署脚本 `run(uint256)` 仅校验该 env `!= 0`，未校验 `code.length` 或 launcher 身份；`initialize` 后若绑错地址（EOA / 错误合约），hook 的所有 `onlyLauncher` 流程（`executePreorderSettlement` / `setPublicSwapResumeTime`）永久失效，UUPS `_initialized` 已置 `1` 锁定、无链上恢复路径（引入 rescue 会重新打开 F-139 retarget 面）。兜底检测点：launcher 侧 `MemeverseLauncherUpgradeable.sol::setMemeverseUniswapHook` 的 back-pointer 校验 `hook.launcher() == address(this)` 在 launcher 配线时 revert，暴露 hook 侧 binding 错配，须在 launcher 打开 registration 前完成该项配线以触发该校验。`[代码已证]`
- Launcher 与所有继承 `ReentrancyGuard` 的合约（`POLendUpgradeable`、`POLSplitterUpgradeable`、`MemeverseUniswapHookUpgradeable`）依赖 EIP-1153 transient storage（`tload`/`tstore` 操作码），编译目标 `evm_version = "prague"`。部署链必须支持 Cancun 或更新硬分叉，否则 `nonReentrant` 修饰符将导致 `invalid opcode` 回退。见 [docs/operations.md](../../operations.md#6-evm-兼容性要求)。`[代码已证]`
...
- 该 yieldDispatcher readiness 为本链接线校验（env 变量 == 本链 launcher 存储值），不校验跨链地址相等。跨链 fee 分发（`MemeverseSettlementImpl.sol::_sendRedeemedFeesCrossChain`）的 OFT `SendParam.to` = 源链 `launcher.yieldDispatcher`，投递到 gov 链，负载性前提为「源链 `launcher.yieldDispatcher` == gov 链上真实部署的 YieldDispatcherUpgradeable 地址」。部署脚本经 CREATE3 两步部署（`script/MemeverseScript.s.sol::_deployYieldDispatcher`）：implementation 用 salt `SALT_YIELD_DISPATCHER_IMPLEMENTATION = "YieldDispatcherImplementation"`，ERC1967Proxy 用原 `SALT_YIELD_DISPATCHER = "YieldDispatcher"` + initialize calldata；CREATE3 地址 = f(OutrunDeployer, deployer, salt) 与 creationCode 无关，故改 UUPS 后 proxy 地址不变（仍等于原 dispatcher 地址），`MEMEVERSE_YIELD_DISPATCHER` env 无需重指向，各链同址三元组前提（OutrunDeployer 跨链同址 ∧ deployer/broadcaster 跨链同址 ∧ nonce 同）保持。`initialize` 每链传同一 `PROTOCOL_TREASURY`（各链 env 配同一值），故 `protocolTreasury` 跨链同址；此为部署约定非合约强制（`MemeverseLauncherUpgradeable.sol::setYieldDispatcher` 仅校验非零；`YieldDispatcherUpgradeable.sol::setProtocolTreasury` 仅 owner、非零校验），错配即协议费滞留、无回收入口、fail-closed（无第三方窃取）。`[代码已证]`
...
- `POLendUpgradeable.creditFactory()` 进 readiness check：脚本 `_readAddress(POLEND, "creditFactory()")` 取值后 `_requireContractCode(..., "POLEND_CREDIT_FACTORY_NOT_READY")`，与 bootstrap/fee sibling 同类（均为用户路径接线指针）。校验"有代码"而非"非零"——`_buildPOLendCreationCode` 在未设 `CREDIT_FACTORY_PROXY` 时会兜底写入 `initialOwner`（非零 EOA），仅 `code.length > 0` 才能拦住占位、阻断 registration 打开。`[代码已证]`
...
- 每个支持的 `uAsset` 都有非零 `fundMetaDatas(uAsset).minTotalFund` 与 `fundMetaDatas(uAsset).fundBasedAmount`，且派生虚拟资产缓冲 `virtualAssets = minTotalFund × fundBasedAmount × 7 / 1000 > 0`（等价 `minTotalFund × fundBasedAmount >= 143`）。
- `POLendUpgradeable.settlementDustStates(uAsset).maxReserve > 0`。
...
- `lzEndpointRegistry` 读回进 readiness check：脚本 typed decode `getLauncherContracts()` 后断言 `launcherContracts.lzEndpointRegistry == LZ_ENDPOINT_REGISTRY`（错误串 `LAUNCHER_REGISTRY_NOT_READY`，`_loadReadinessEnv` 从 `LZ_ENDPOINT_REGISTRY` env 装载），与其余五个依赖 pin（registrar / proxyDeployer / yieldDispatcher / polend / polSplitter）同型；捕获跨会话/双角色部署下 launcher 烧入值与当日 env 漂移（registry 换 nonce 重部署或 env 抄错）；该 env pin 另经 `_requireContractCode(LZ_ENDPOINT_REGISTRY, "REGISTRY_CODE_NOT_READY")` 校验有代码（该码检位于 `script/MemeverseScript.s.sol::_requireRegistrationCenterReady`，onboard 路径同样生效），与其余依赖码检同型，拦截「launcher 烧入 codeless 预测地址 + readiness env 同错值」的等值穿透；registry 为 initialize-only（setter 已移除），错配无法运行时修复、只能重部署，故须在 registration 打开前拦截。受影响运行时路径：`MemeverseLaunchImpl.sol::_lzConfigure`（远端注册 `lzEndpointIdOfChain`→`setPeer`）与 `MemeverseSettlementImpl.sol::_sendRedeemedFeesCrossChain`（跨链 fee 分发 EID 解析），及 view 报价路径 `MemeverseFeePreviewReader.sol::quoteDistributionLzFee`（读 `contracts.lzEndpointRegistry` 解析 govEndpointId）。`[代码已证]`
- registration center `LZ_ENDPOINT_REGISTRY` 读回进 readiness check：`script/MemeverseScript.s.sol::_requireRegistrationCenterReady`（由 `::_openSupportedUAssetsAfterReadiness` 与 `::onboardUAsset` 两条 registration 打开路径调用）断言经 `_readAddress(registrationCenter, "LZ_ENDPOINT_REGISTRY()")` staticcall 读回（接口未声明该 getter，selector 与生产合约 public immutable getter 一致）的值 `== LZ_ENDPOINT_REGISTRY`（错误串 `REGISTRATION_CENTER_REGISTRY_NOT_READY`，预期值由 `_loadReadinessEnv` 从 `LZ_ENDPOINT_REGISTRY` env 装载，与 `DAY()` 读回同型）——center 为构造部署（非 proxy），registry immutable 错值永久烧入字节码、无 setter 可救，须在 registration 打开前拦截；受影响运行时路径为 `MemeverseRegistrationCenter.sol::quoteSend`。`[代码已证]`
- `MEMEVERSE_OMNICHAIN_INTEROPERATION` 进 readiness check：interoperation 部署地址经新必需 env 装载（`_loadScriptEnv` / `_loadReadinessEnv` 双装载，经 `_optionalEnvAddress` 装载、与 POLEND/POLSPLITTER 同型：`run()` 部署 `MemeverseOmnichainInteroperation`（`script/MemeverseScript.s.sol::_deployMemeverseOmnichainInteroperation`）后回写实际地址、同 run 的 readiness 用回写值，无需预填；standalone readiness 必须提供该 env，缺失时以 `INTEROPERATION_CODE_NOT_READY` 具名失败；env 值无需等于实际部署地址（回写覆盖）），`_requireDeploymentReady` 对其 `_requireContractCode(..., "INTEROPERATION_CODE_NOT_READY")` 码检并断言 `_readAddress(..., "LZ_ENDPOINT_REGISTRY()") == LZ_ENDPOINT_REGISTRY`（错误串 `INTEROPERATION_REGISTRY_NOT_READY`，预期值同源 `LZ_ENDPOINT_REGISTRY` env）——interoperation 为构造部署，registry immutable 错值永久烧入、无 setter 可救，须在 registration 打开前拦截；受影响运行时路径为 `MemeverseOmnichainInteroperation.sol::memecoinStaking` 治理链在异链时的跨链 OFT send，及 view 报价 `MemeverseOmnichainInteroperation.sol::quoteMemecoinStaking`（治理链在异链时，经共享 `_buildStakingSendParam` 消费同一 immutable）。同 run 部署流中该读回为自洽校验（回写值与构造参数同源）；跨链 pin 一致性由各链独立运行 readiness 保证，不在单链闸门覆盖内。`[代码已证]`
- registry pairs 内容探针进 readiness check：脚本 `_requireDeploymentReady` 对 `script/MemeverseScript.s.sol::_chainsInit` 装载的单源链表（`_chainsInit` 同时吸收 `omnichainIds` 赋值，链表与 `endpointIds` env 装载同源单点）全 9 链逐一断言 `ILzEndpointRegistry(LZ_ENDPOINT_REGISTRY).lzEndpointIdOfChain(chainId) == endpointIds[chainId]`（错误串 `REGISTRY_PAIR_NOT_READY`），并断言本地锚点 `lzEndpointIdOfChain(uint32(block.chainid)) == 本地 endpoint 的 eid() 读回`（错误串 `REGISTRY_LOCAL_EID_NOT_READY`，env 无关的链上真值锚、覆盖本链切片）。覆盖三个漂移向量：跨会话 env 分叉、registry 换 nonce 重部署后旧实例存活（env pin 仍指旧地址、其表内容陈旧）、开闸前 owner 经 `LzEndpointRegistry.sol::setLzEndpointIds` 重指表项。覆盖面 carve-out：内容探针与本地锚点覆盖 `run()` 与 `::openSupportedUAssetsAfterReadiness` 两条路径；`::onboardUAsset` 只调 `_requireRegistrationCenterReady`、不经 `_requireDeploymentReady`，内容探针与本地锚点在该路径不触发（规范流程下首次开闸必经 `::openSupportedUAssetsAfterReadiness`——launcher fundMetaDatas UETH/UUSD 检查强制——探针仍先行校验，仅非规范直走 `::onboardUAsset` 的用法绕过）。盲区声明：单会话 run() 内探针恒真（`script/MemeverseScript.s.sol::_deployLzEndpointRegistry` 与探针同源读同一 `endpointIds` env，自洽校验，价值在跨会话 / standalone readiness）；开闸后 owner 重指不在闸门射程；远端链同源同错（env 全链抄错同一 eid）仍盲，仅本链切片由锚点覆盖。残余观察：registrar / registration center 部署期 `setPeer` 直接消费 `endpointIds` env、不经 registry（`script/MemeverseScript.s.sol::_deployRegistrationCenter` / `::_deployMemeverseRegistrar`），远端 eid 同源错配任何 registry 探针不覆盖，属部署清单人工核对步骤。`[代码已证]`
- registry 部署期预测-vs-实际断言闸门（`REGISTRY_DEPLOY_MISMATCH`）：`script/MemeverseScript.s.sol::_deployLzEndpointRegistry` 在 CREATE3 部署 LzEndpointRegistry 后、`setLzEndpointIds` 写入 pairs 前，`require` 部署返回地址 == `LZ_ENDPOINT_REGISTRY` env pin。操作前提：执行该部署步骤前，须先经 `script/MemeverseScript.s.sol::_getDeployedLzEndpointRegistry(nonce)` 取当前 nonce 的 CREATE3 预测地址预填该 env。失败形态：forge 全量模拟在 broadcast 前中止整个 run——无链上孤儿实例、无部分 pairs 写入。`[代码已证]`
