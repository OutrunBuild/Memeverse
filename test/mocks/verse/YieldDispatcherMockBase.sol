...
abstract contract YieldDispatcherMockBase {
    ...
    /// @param dispatcher_ The YieldDispatcherUpgradeable whose composeStates to read.
    ...
    function _checkComposeProbes(address token_) internal view {
        if (composeProbeDispatcher != address(0)) {
            require(
                YieldDispatcherUpgradeable(composeProbeDispatcher).composeStates(token_, composeProbeGuid)
        ...
        if (composeProbeReleasedDispatcher != address(0)) {
            require(
                YieldDispatcherUpgradeable(composeProbeReleasedDispatcher)
                        .composeStates(token_, composeProbeReleasedGuid) == IComposeState.ComposeState.Released,
