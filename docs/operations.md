...
- `YieldDispatcher.settlePendingCompose` 的兜底结算语义是重跑与正向路径完全一致的结算（approve+pull 到 message 编码的 receiver：vault/governor；EOA receiver 则 burn），不是退回调用者——此"完全一致"仅对 dispatcher 成立；`OmnichainMemecoinStaker.settlePendingCompose` 语义不同（恒释放裸币、不建仓），见 §3.13.1
...
| dispatcher EOA-burn（`burn` 直销）| ~38000 | 135000 | 足够 |
