...
- `setFacet` 在链上只验证 `onlyOwner`、已知 role、非零地址、候选地址有字节码，以及 `ImmutableState(facet).poolManager()` 与 Hook `poolManager` 一致；它不验证完整 role ABI、共享 storage layout 或 `onlyViaRouter`。**该缺口由部署/升级门禁在链下闭合**：owner（multisig）在发起 `setFacet` 前必须通过 `test/swap/FacetGuardCompliance.t.sol` 与 `test/swap/MemeverseDiamondFacets.t.sol` 的 `DirectFacetCallForbidden` 直调断言（源码字面量+运行时回退双重钉死），并在 `script/DeployMemeverseHookProxy.s.sol` 等部署脚本中对候选地址做离线探针；已证非守卫 facet 的直接 `CALL` 仅落在 facet 自身账户的零值存储（`address(this)==facet` 时 erc7201 锚与 hook 存储天然隔离）、无余额可动且 `poolManager.take/settle` 瞬态轧平必 revert，hook 账本/资金不可达，且 owner 经合规恶意 facet 或 UUPS `upgradeToAndCall` 已持有更强权力，故 Router 保持极简不在链上追加临时 canary。
...
- facet **必须**继承 `FacetGuard` 并保留 `onlyViaRouter` 的 direct-`CALL` 拒绝（除 `ImmutableState.poolManager()` 纯 view getter 外，全部外部逻辑入口均带该修饰符）。该 guard 只区分 direct `CALL` 与 `delegatecall`，不认证 delegatecall 的宿主一定是指定 Hook proxy；是否携带由上述链下门禁保证，并由 `test/swap/FacetGuardCompliance.t.sol` 以源码字面量与运行时直调回退双重钉死。
...
- 存储布局必须与共享 `outrun.storage.MemeverseLauncher` namespace 的 `IMemeverseLauncherStorage::MemeverseLauncherStorage` struct 保持同步：升级该 struct 时三者必须同步重编（该约束已由 §4 的 sibling 通用段覆盖，此处不重复展开）。
...
- `deployCaller` 是执行 CREATE3 / proxy 部署的调用者，`initialOwner` 是 Launcher proxy 初始化后的 owner；两者显式拆分：单角色（`deployCaller == initialOwner`）时脚本在部署中直接写入 `setFundMetaData`，双角色（`deployCaller != initialOwner`）时由 `initialOwner` 单独调用，部署模式细节唯一权威见 [docs/spec/verse/deployment.md §6](verse/deployment.md)。测试 harness 通过覆盖 `_beginMemeverseLauncherOwnerExecution` 实现 `vm.startPrank` 以在单交易内测试双角色路径。`[代码已证]`
...
- readiness 检查不能只验证 implementation 或 proxy code 存在：依赖 pin 与 back-reference、`fundMetaDatas[uAsset]`、`settlementDustStates(uAsset).maxReserve`、`launchImpl` / `settlementImpl` / `liquidityImpl` / `feePreviewReader` / `creditFactory` 有代码、router↔hook PoolManager 一致性（`ROUTER_POOL_MANAGER_NOT_READY`）等检查项全集的唯一权威 runbook 见 [operations.md §3.9.7](../operations.md)；readiness 经一次 typed decode `getLauncherContracts()` 读出全部依赖字段（launcher 无对应 direct config getter，`polend()` 为保留的 direct getter）。`[代码已证]`
  - 证据：`script/MemeverseScript.s.sol::_checkMemeverseLauncherDeployment`、`::_requireDeploymentReady`、`::_requireSwapReady`
...
- `MemeverseLaunchImpl` / `MemeverseSettlementImpl` / `MemeverseLiquidityImpl` 三个 sibling 同属 Launcher facade 受信 `delegatecall` 目标集（Launcher proxy 受信代码集），共同事实如下。`[代码已证]`
  - sibling 与 Launcher facade 共享同一 ERC-7201 namespace `outrun.storage.MemeverseLauncher` 与 `IMemeverseLauncherStorage` struct；升级 `MemeverseLauncherStorage` struct 时三者必须用相同 struct 同步重编，否则 facade 经 delegatecall 调任一 sibling 时读写错位；各 sibling 仅写各自业务边界对应的 `IMemeverseLauncherStorage` 语义字段，零越界写其他 storage。
  - facade 各入口 inline `require(*Impl != address(0), *ImplNotSet())` 前置守卫（`LaunchImplNotSet` / `SettlementImplNotSet` / `LiquidityImplNotSet`），防零地址 void `delegatecall` 静默成功、入口逻辑不执行却推进状态。
  - sibling 继承 `DelegatecallOnly`，EOA 直调时 `onlyDelegatecall` 守卫（immutable 变量 `_self = address(this)` 在构造期求值锚定 sibling 部署地址、其值嵌入 sibling 自身 bytecode，delegatecall 上下文切换下不被 proxy 覆盖）显式 revert `DelegatecallOnlyCall`。
  - sibling 为空 constructor、无 `Initializable`、无自身 storage，自身 storage 永久未初始化，亦不构成可利用路径。
  - 差异仅在 owner 替换入口（均为 `onlyOwner`，与 Launcher 其他 admin setter 一致）：
    - `MemeverseLaunchImpl`：`setLaunchImpl`
    - `MemeverseSettlementImpl`：`setSettlementImpl`
    - `MemeverseLiquidityImpl`：`setLiquidityImpl`
  - 证据：`src/verse/MemeverseLaunchImpl.sol`；`src/verse/MemeverseSettlementImpl.sol`；`src/verse/MemeverseLiquidityImpl.sol`；`src/verse/MemeverseLauncherUpgradeable.sol::setLaunchImpl`、`::setSettlementImpl`、`::setLiquidityImpl`；`src/verse/interfaces/IMemeverseLauncher.sol::LaunchImplNotSet`、`::SettlementImplNotSet`、`::LiquidityImplNotSet`
