...
- 保护窗口在正常 `Locked -> Unlocked` 时按固定 24 小时写入。`launcher` 由 hook initialize 固化（initializer write-once），不可 retarget，binding 运行时不可偏离真实 Launcher proxy；详见 INV-12 / operations §3.8。
