...
| `ProtocolTreasuryChanged(address indexed oldTreasury, address indexed newTreasury)` | `YieldDispatcher` | `setProtocolTreasury` 成功时（owner 旋转协议金库，`onlyOwner`） | `YieldDispatcher.sol::setProtocolTreasury` 在 owner 旋转 `protocolTreasury` 后 emit；`oldTreasury` = 旧金库，`newTreasury` = 新设的非零金库。对账：标识 UASSET no-code receiver 路由落点变更——`protocolTreasury` 是伪造/自伤 UASSET no-code 帧及未来 no-code UASSET 投递的转入目标，索引器据此追踪可配置协议金库旋转。`[代码已证]` |
