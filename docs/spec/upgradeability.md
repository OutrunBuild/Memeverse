...
- readiness 经一次 typed decode `getLauncherContracts()` 读出全部依赖字段（launcher 无 `memeverseRegistrar()` / `memeverseProxyDeployer()` / `yieldDispatcher()` / `polSplitter()` direct config getter；`polend()` 为保留的 direct getter，不在 `LauncherContracts`）。`[代码已证]`
