...
| `POLSplitterUpgradeable` | `ERC1967Proxy` + UUPS | `initialize(initialOwner, _launcher)` | `_authorizeUpgrade(...) => onlyOwner` | `src/polend/POLSplitterUpgradeable.sol`（`_authorizeUpgrade`） |
...
- Launcher 保存的是 `POLendUpgradeable` / `POLSplitterUpgradeable` 的 proxy 地址，当前规范不提供 setter、地址级替换、迁移或降级零地址模式；这只约束 proxy 地址本身，不否定 proxy 实现升级。`POLendUpgradeable` 与 `POLSplitterUpgradeable` 均为 UUPS，`_authorizeUpgrade(...)` 由 `onlyOwner` 放行。`[代码已证]`
