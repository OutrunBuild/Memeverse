...
/// @title Direct preorder settlement invariant
/// @notice Invariant fuzz suite over the hook's launcher-bound `executePreorderSettlement` entrypoint,
///         asserting treasury/handler/recipient balance accounting matches the handler's independently
///         computed expectations, and that settlement never breaks the public quote fee floor or
///         rewrites the pool launch timestamp.
...
contract DirectPreorderSettlementHandler is Test {
    ...
    function settlementSwap(uint256 amountSeed, bool zeroForOne, bool useCurrency0AsFeeSide) external {
        ...
        // Output-side fee divides the plain BPS_BASE: the fee base here is the post-LP-fee output,
        // so no double-count adjustment applies (unlike ordinary swaps' effective-rate denominator).
