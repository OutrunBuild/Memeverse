...
- `docs/superpowers/**`, where present, holds superpowers-workflow design drafts and plan artifacts. It is never product truth: untracked by Git (gitignored where configured), unmaintained, and may be stale or deleted at any time. Product docs must not reference it — no `docs/superpowers/**` path citations from product docs (`docs/**` except `docs/prompts/**`, `docs/audits/**`, `docs/review/**`) or `README.md`. Harness-control files may name it only to exclude it from a truth scope.
