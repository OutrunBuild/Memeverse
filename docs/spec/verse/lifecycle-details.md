...
本文用用户视角、资金流视角和模块协作视角，把 Memeverse 的完整生命周期串起来。
...
- 杠杆辅助流动性结算零滑点为活性优先取舍: `MemeverseLiquidityImpl.sol::_removeAuxiliaryLiquidityIfNonZero` 固定 `0,0` 以保证 `Locked→Unlocked` 解锁必达(紧下限可被持续推价 grief 致 DOS, 见 invariants.md INV-07A), 推价缺口由 `settlementDustReserve` 有界兜底【INV-13】, 后腿 24h 阻断消除原子三明治
- `UNLOCK_PROTECTION_WINDOW` 是正常 `Locked -> Unlocked` 时写入的固定 24 小时窗口；`launcher` 由 hook initialize 固化（initializer write-once），不可 retarget，binding 运行时不可偏离真实 Launcher proxy，无需恢复操作；详见 INV-12 / operations §3.8。
