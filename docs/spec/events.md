...
本节事件面由 [docs/spec/polend/README.md](polend/README.md) 的目标事件规格驱动、现已全部落地为 `[代码已证]`；current vs target 缺口已清零。
...
| `OFTProcessed` | `YieldDispatcherUpgradeable` | OFT compose 到账处理（`lzCompose`，真实 compose guid）；同链费用分发 `distributeSameChain`（guid 恒为 `bytes32(0)`，launcher 同链分支调用） | 收益路由或 burn 结果；`burnedAtDispatcher=true` 仅表示 dispatcher 在 EOA receiver 分支执行了 MEMECOIN `burn` 调用；no-code receiver 按 tokenType 分流（MEMECOIN → burn、UASSET → `protocolTreasury` 且恒为 `false`）；对账语义（fallback 吸收、零金额/自引用分支）见 [operations.md §3.13](../operations.md)。`[代码已证]` |
| `ComposeSettled(bytes32 indexed guid, address indexed token, address indexed receiver, TokenType tokenType, uint256 amount, bool burnedAtDispatcher)` | `YieldDispatcherUpgradeable` | 已投递未执行 compose 兜底结算完成 | `settlePendingCompose` 成功时触发；`tokenType` 为消息解码的结算类型（MEMECOIN→yieldVault / UASSET→governor），对账可直接按事件字段分账、无需 receiver 角色映射。`burnedAtDispatcher` 语义与伪造帧边界（`token` 键不保证真实桥接 token，对账须按已知 token 地址过滤）见 [operations.md §3.13](../operations.md)。`[代码已证]` |
| `ComposeRejected(bytes32 indexed guid,address indexed token,uint256 amount)` | `YieldDispatcherUpgradeable` / `OmnichainMemecoinStakerUpgradeable` | 畸形 compose payload 被消费（无资金移动） | 畸形 compose payload 被消费（置 `Settled`、不结算、无资金移动）的信号；两个 composer 同签名发出，`amount` 语义按 composer 分流（dispatcher 短帧报 0、staker 恒为可读 amountLD）。完整触发面、帧长分叉与自伤边界见 [operations.md §3.13 / §3.13.1](../operations.md)。`[代码已证]` |
...
| `OmnichainMemecoinStaking` / `OmnichainMemecoinStakingProcessed` | interoperation/staker | 发起远端 staking / 远端处理完成 | 跨链 staking 追踪。对账注意：①receiver 为 staker 自身或无出口合约时 `Processed` 事件可自伤伪造、资金零移动，须按 receiver 过滤后核对余额；②截断时实际质押 = `amountSentLD`、余量同交易退回（核对 `amountSentLD + remainder == amount`）；③`yieldVault == address(0)` 有合规无 vault 与 vault word 脏位降级两源，须重解码 message 判别。完整机制见 [operations.md §3.13.1](../operations.md) / [interoperation-details §4.5](interoperation/interoperation-details.md)。 |
