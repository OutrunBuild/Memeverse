...
- 该 yieldDispatcher readiness 为本链接线校验（env 变量 == 本链 launcher 存储值），不校验跨链地址相等。跨链 fee 分发（`MemeverseSettlementImpl.sol::_sendRedeemedFeesCrossChain`）的 OFT `SendParam.to` = 源链 `launcher.yieldDispatcher`，投递到 gov 链，负载性前提为「源链 `launcher.yieldDispatcher` == gov 链上真实部署的 YieldDispatcher 地址」。部署脚本经 CREATE3（`script/MemeverseScript.s.sol::_deployYieldDispatcher`，salt = `keccak256("YieldDispatcher", nonce)`，地址 = f(OutrunDeployer, deployer, salt)）按构造使各链同址，同址三元组前提 = OutrunDeployer 跨链同址 ∧ deployer/broadcaster 跨链同址 ∧ nonce 同；此为部署约定非合约强制（`MemeverseLauncher.sol::setYieldDispatcher` 仅校验非零），错配即协议费滞留、无回收入口、fail-closed（无第三方窃取）。`[代码已证]`
