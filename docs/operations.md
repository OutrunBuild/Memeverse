...
4. center 面（**最后**开启，完成标志）：`MemeverseRegistrationCenterUpgradeable.sol::setSupportedUAsset(uAsset, true)`（onlyOwner）。漏配 → 注册参数校验 `MemeverseRegistrationCenterUpgradeable.sol::_registrationParamValidation` 在 `supportedUAssets[uAsset]` 未配置时 revert `InvalidUAsset`，注册直接被拒；前三步 readiness 断言通过前不得开启。`[代码已证]`

补充说明：

- 四个面全是 onlyOwner setter，无任何自动接线，遗漏只能在部署/注册时暴露；注册事务是单事务原子回滚（token 克隆一并回滚），可干净重试，无残留状态。
- 脚本 readiness 断言复用 canonical 对（UETH/UUSD）已验证的 `_requireFundMetaDataReady` / `_requireReserveReady` 语义，泛化到任意 uAsset；`setMaxSettlementDustReserve` 仅经 `onboardUAsset` 脚本路径写入，带外部署易漏配，是冷启动漏配的高发面。
...
- `setFundMetaData` 为全局活读配置：owner 调整立即影响 Genesis 期 verse 的启动门槛、铸币比、V 缓冲与杠杆上限（已 `Locked` verse 不受影响）；四处决策点与方向性影响见 [docs/spec/verse/config-matrix.md §2 注](spec/verse/config-matrix.md)。

#### 3.1.2 registrar 指针更换与 inbound 解锁 runbook

center（`MemeverseRegistrationCenterUpgradeable`）的 registrar 是 owner 可变的 storage 指针（`MemeverseRegistrationCenterUpgradeable.sol::setMemeverseRegistrar`，onlyOwner、零值具名拒绝、emit `SetMemeverseRegistrar(old, new)`）。更换 registrar（registrar 合约损坏/密钥事件后的 deadlock 解锁）时必须成对操作：

1. owner 调 `setMemeverseRegistrar(newRegistrar)`：本链路径（`registration` -> `MemeverseRegistrarAtLocal.localRegistration`）立即跟随新指针，仅此一步即恢复本链注册。
2. 逐个相关 eid 调 `setPeer(eid, bytes32(newRegistrar))`：OApp 基座在进入 `MemeverseRegistrationCenterUpgradeable.sol::_lzReceive` **之前**强制 `peers[srcEid] == origin.sender`，peer 未换时 inbound（异链 registrar -> center 的跨链注册）在 peer 校验即失败（`OnlyPeer`），到不了 `_lzReceive` 的 origin 校验。
3. 中间态（指针已换、peer 未换）下 inbound 恒 fail-closed，但两侧错误不同——新 origin → `OnlyPeer`（peer 检查先拦截）；旧 origin → 先过 peer 检查、在 `MemeverseRegistrationCenterUpgradeable.sol::_lzReceive` origin 校验处 `PermissionDenied`。监控仅盯 `OnlyPeer` 会漏判旧 origin 流量。无绕过面，但异链注册保持不可用直到步骤 2 完成；每个会向 center 发送注册消息的远端链各需一次 `setPeer`。
4. 验证：`memeverseRegistrar()` 读回 == 新地址；对每个相关 eid 确认 peer 已更新；`SetMemeverseRegistrar` 事件与各 `PeerSet` 事件均已上链。
...
- 治理链在异链：先 quote，`msg.value` 必须精确匹配，走 OFT 到 `OmnichainMemecoinStakerUpgradeable`
...
- Launcher proxy 初始化时保存 `POLendUpgradeable` 与 `POLSplitterUpgradeable` 的 proxy 地址，当前规范不支持地址级替换，也不支持降级为零地址模式；如需支持，必须先给出显式迁移设计。`[代码已证]`
...
- `launcher` 由 hook initialize 固化、不可 retarget，无需恢复操作；resume time 写入仅正常 Locked→Unlocked 路径由真实 Launcher 执行。
...
各合约 proxy / implementation salt label 与 canonical 地址推导的单一事实表见 [spec/verse/deployment.md §6](spec/verse/deployment.md)；本表聚焦升级操作：部署来源、授权门控与特殊约束。

| 合约 | proxy 来源 / proxy salt label | implementation 部署来源 | 当前可证 implementation salt label | 授权门控 | 特殊约束 |
| --- | --- | --- | --- | --- | --- |
| **UUPS 可升级** | | | | | |
| `MemeverseLauncherUpgradeable` | `MemeverseScript._deployMemeverseLauncher`; proxy salt label 见 deployment.md §6 | `MemeverseScript._deployMemeverseLauncher` 内部署 | 见 deployment.md §6 | `onlyOwner` | 存储 `polend`/`polSplitter` proxy 地址，不可运行时替换 |
| `POLendUpgradeable` | `MemeverseScript._deployPOLend`; proxy salt label 见 deployment.md §6 | `MemeverseScript._deployPOLend` 内部署 | 见 deployment.md §6 | `onlyOwner` | 存储 `launcher`/`splitter` 地址；`leveragedDebtFactor` 技术上限 `uint128.max * 1e18` |
| `POLSplitterUpgradeable` | `MemeverseScript._deployPOLSplitter`; proxy salt label 见 deployment.md §6 | `MemeverseScript._deployPOLSplitter` 内部署 | 见 deployment.md §6 | `onlyOwner` | 存储 `launcher`/`polend` 地址；初始化时读 `launcher.polend()` |
| `YieldDispatcherUpgradeable` | `MemeverseScript._deployYieldDispatcher`; proxy salt label 见 deployment.md §6 | `MemeverseScript._deployYieldDispatcher` 内部署 | 见 deployment.md §6 | `onlyOwner`（经 `YieldDispatcherUpgradeable.sol::_authorizeUpgrade`） | `constructor() { _disableInitializers(); }`；CREATE3 两步部署（impl + ERC1967Proxy + `YieldDispatcherUpgradeable.sol::initialize` calldata），proxy 地址 = f(OutrunDeployer, deployer, salt) 与 creationCode 无关 |
| `MemeverseRegistrationCenterUpgradeable` | `MemeverseScript._deployRegistrationCenter`; CREATE3 两步部署（镜像 `_deployYieldDispatcher`：impl + ERC1967Proxy，deploy+initialize 经 proxy constructor data 原子完成） | `MemeverseScript._deployRegistrationCenter` 内部署 | 见 `script/MemeverseScript.s.sol`（以部署脚本为唯一真相源） | `onlyOwner`（经 `MemeverseRegistrationCenterUpgradeable.sol::_authorizeUpgrade`） | impl `constructor(address _lzEndpoint)` 把 endpoint 烧入 implementation immutable（OAppUpgradeable 基座）；升级经 owner `upgradeToAndCall`，`_authorizeUpgrade` 先无代码守卫（revert `UpgradeTargetCodeNotReady`），再校验 `IOAppCore(newImplementation).endpoint() ==` 当前 endpoint（不匹配 revert `UpgradeEndpointMismatch`，命名错误可 grep、guard 拒绝时升级交易 revert 而非静默换绑）；有代码但 getter 缺失或调用 revert 时具名 revert `UpgradeEndpointUnreadable`（不落裸 ABI-decode revert，与无代码守卫同属可 grep 的诚实失败）；proxy storage（`outrun.storage.MemeverseRegistrationCenter`）跨升级保留、字段只允许尾部追加；owner 基座为 OZ `OwnableUpgradeable`（OApp 基座内建、单 owner 槽），`renounceOwnership` 经共享抽象基座 `OutrunOAppUpgradeable`（`src/common/omnichain/oapp/OutrunOAppUpgradeable.sol`，覆盖为恒 revert `OwnershipRenounceDisabled`，镜像 Outrun ownable 家族的基座级 never-renounceable 编码）恒 revert；implementation 直收 ETH 被 `receive()` 上的 `DelegatecallOnly::onlyDelegatecall` 守卫拒绝（`src/common/access/DelegatecallOnly.sol`，构造期固化自身地址的 immutable mixin——直发 revert `DelegatecallOnlyCall` 原路退回，不再滞留在 owner 恒 0 的 implementation 上；proxy 语境收款不受影响，LayerZero native fee 退款路径不变；selfdestruct 强推不可防御、同既有边界） |
| `OmnichainMemecoinStakerUpgradeable` | `MemeverseScript._deployOmnichainMemecoinStaker`; CREATE3 两步部署（镜像 `_deployYieldDispatcher`：impl + ERC1967Proxy，deploy+initialize 经 proxy constructor data 原子完成） | `MemeverseScript._deployOmnichainMemecoinStaker` 内部署 | 见 `script/MemeverseScript.s.sol`（以部署脚本为唯一真相源） | `onlyOwner`（经 `OmnichainMemecoinStakerUpgradeable.sol::_authorizeUpgrade`） | `constructor() { _disableInitializers(); }`，零值检查在 `initialize(initialOwner, _localEndpoint)`（具名零检查）；升级经 owner `upgradeToAndCall`，无 endpoint guard（`localEndpoint` 是 proxy storage 而非 implementation immutable）；proxy storage（`outrun.storage.OmnichainMemecoinStaker`，含 `composeStates` 互斥锁）跨升级保留、字段只允许尾部追加；owner 仅持升级授权——升级权等同对滞留 bridged memecoin 的托管权（与 dispatcher 同一接受的 residual） |
| **UUPS 可升级（diamond Router）** | | | | | |
| `MemeverseUniswapHookUpgradeable` | `DeployMemeverseHookProxy.getPredictedProxy(..., nonce, hookOwner, hookTreasury, poolManager)`；内部 `_selectProxySalt` 选择 nonce-scoped hook-flag proxy salt，mining salt 公式见 deployment.md §6 注 | `DeployMemeverseHookProxy` 通过 `new MemeverseUniswapHookUpgradeable(poolManager)` 部署 | 见 deployment.md §6 | `onlyOwner`（Hook `owner()`，经 UUPS `_authorizeUpgrade`） | diamond Router：`poolManager` 不在 proxy storage 中，是字节码级绑定；callback/fee/settlement logic 外移到 SwapFacet / DynamicFeeFacet / SettlementFacet（共享 Router storage，经 Router entry `delegatecall`）；3 facet 地址存 Router storage，`setFacet(role, addr)` onlyOwner 独立升级每个 facet；3 facet 部署时必须传与 hook 同一个 `poolManager` |
| **治理代理可升级** | | | | | |
| `MemecoinDaoGovernorUpgradeable` | `MemeverseProxyDeployer.deployGovernorAndIncentivizer`; proxy salt 公式见 upgradeability.md §4 | `MemeverseScript._deployMemecoinGovernorImplementation` | 见 `script/MemeverseScript.s.sol`（以部署脚本为唯一真相源） | `onlyGovernance` | 需走 OZ Governor 提案流程；`_authorizeUpgrade` 由 Governor 合约内部 `_governanceCall` 放行 |
| `GovernanceCycleIncentivizerUpgradeable` | `MemeverseProxyDeployer.deployGovernorAndIncentivizer`; proxy salt 公式见 upgradeability.md §4 | `MemeverseScript._deployImplementation` | 见 `script/MemeverseScript.s.sol`（以部署脚本为唯一真相源） | `onlyGovernance` | 实际校验 `msg.sender == _governor`（即 Governor proxy 地址） |
...
| `MemeverseRegistrationCenterUpgradeable` | `erc7201:outrun.storage.MemeverseRegistrationCenter` | `outrun.storage.MemeverseRegistrationCenter` |
| `OmnichainMemecoinStakerUpgradeable` | `erc7201:outrun.storage.OmnichainMemecoinStaker` | `outrun.storage.OmnichainMemecoinStaker` |
...
namespace 与共享 storage 的规则性归属见 [docs/spec/upgradeability.md §2](spec/upgradeability.md)；本表为升级前 storage 兼容性验证的操作辅助。
...
2. 部署到目标链。不要默认所有 UUPS 合约都有 implementation salt label；按 3.9.1 的 `implementation 部署来源` 执行：
   - `MemeverseLauncherUpgradeable`：按 deployment.md §6 的 implementation salt label 部署新的 implementation；不要重跑会重新部署同一 proxy salt 的完整 proxy 部署步骤。
   - `MemecoinDaoGovernorUpgradeable`：使用 `MemeverseScript._deployMemecoinGovernorImplementation` 的 deployment source（salt label 以 `script/MemeverseScript.s.sol` 为唯一真相源）。
   - `GovernanceCycleIncentivizerUpgradeable`：使用 `MemeverseScript._deployImplementation` 中的 incentivizer implementation 部署逻辑（salt label 以 `script/MemeverseScript.s.sol` 为唯一真相源）。
   - `MemeverseUniswapHookUpgradeable`：只复用 `DeployMemeverseHookProxy` 的 implementation 部署逻辑，不重跑 proxy 部署分支；必须传入与当前 proxy 一致的 `POOL_MANAGER`。升级 Router implementation 不替换 facet 地址（facet 独立存于 Router storage，经 `setFacet` 升级）；若需单独升级某个 facet，部署新 facet（SwapFacet / DynamicFeeFacet / SettlementFacet 均须传同一 `POOL_MANAGER`）后调 `hook.setFacet(role, newAddr)`；链上 `setFacet` 经 `_requireFacetPoolManager` 对三者统一强制校验，不匹配 revert `FacetPoolManagerMismatch`；facet 有代码但 `poolManager()` getter 缺失/revert 时具名 revert `FacetPoolManagerUnreadable`（与升级守卫同族的 try/catch 折叠）。
   - `POLendUpgradeable`：使用 `MemeverseScript._deployPOLend` 中的 implementation 部署逻辑，按 deployment.md §6 的对应 implementation salt label。
   - `POLSplitterUpgradeable`：使用 `MemeverseScript._deployPOLSplitter` 中的 implementation 部署逻辑，按 deployment.md §6 的对应 implementation salt label。
...
`poolManager` 地址不在 proxy storage 中，它是字节码级绑定（在 implementation constructor 中设置的 immutable 或 constructor 参数），故升级守卫经新 implementation 的 getter 与当前值比对；守卫链完整语义（无代码守卫 → `UpgradePoolManagerMismatch` → `UpgradePoolManagerUnreadable`、运维护栏定性）见本节上文。若绕过该守卫（如恶意 owner 伪造 getter），hook 回调将指向错误目标，所有 swap 和流动性操作会永久失效。
...
#   [2] lzEndpointRegistry     == 预期 LzEndpointRegistry 地址
#   [3] yieldDispatcher        == 预期 YieldDispatcher 地址（readiness 错误串 LAUNCHER_YIELD_DISPATCHER_NOT_READY；本链接线校验：env == 本链 launcher 存储值，不校验跨链地址相等）
#   [4] memeverseRegistrar   == 预期 Registrar 地址
#   [5] memeverseProxyDeployer == 预期 ProxyDeployer 地址（readiness 错误串 LAUNCHER_PROXY_DEPLOYER_NOT_READY）
...
# 人工核对（registry 探针不覆盖的残余）：逐链 *_EID env 对照 LayerZero 官方 EID 表——registrar / center 部署期 setPeer 直消费该 env（MemeverseScript.s.sol::_deployRegistrationCenter / ::_deployMemeverseRegistrar，不经 registry），远端 eid 同源错配漏检
```
...
> readiness 经一次 typed decode `getLauncherContracts()` 读出全部依赖字段（launcher 无 `memeverseRegistrar()` / `memeverseProxyDeployer()` / `yieldDispatcher()` / `polSplitter()` direct config getter；`polend()` 为保留的 direct getter，不在 `LauncherContracts`）。

`creditFactory` 进 readiness check：POLendUpgradeable 的 `creditFactory` 指针由脚本 `_readAddress(POLEND, "creditFactory()")` 取值后 `_requireContractCode` 校验有代码，与 bootstrap/fee sibling 同类（用户路径接线指针——`leveragedGenesisWithCredit` 经 `IGenesisCreditFactory(creditFactory).creditOf(uAsset)` 解析 GenesisCredit）；未接线或被占位 owner 兜底（`_buildPOLendCreationCode` 在未设 `CREDIT_FACTORY_PROXY` 时写入的 EOA，无代码）时 readiness 失败、阻断 registration 打开。`cast call $POLEND_PROXY "creditFactory()(address)" --rpc-url $RPC` 应返回非零且有代码的 factory 地址。校验口径为有代码而非非零——owner 兜底占位是 EOA（非零但无代码，能蒙过零检查）；未接线时首次 `leveragedGenesisWithCredit` 对无代码地址 staticcall 返空并 `abi.decode` revert，credit 路径静默阻断至 `setCreditFactory`。`[代码已证]`

`staker` 进 readiness check：`OMNICHAIN_MEMECOIN_STAKER` 由脚本 `_requireContractCode` 校验有代码（错误串 `STAKER_CODE_NOT_READY`）——dispatcher 已有三重检查（code/双向接线）而 staker readiness 校验有代码，拦截 env 配错（EOA/错合约）时 `memecoinStaking` 远端路径静默指向坏 composer 的配置错误。`cast call $OMNICHAIN_MEMECOIN_STAKER --rpc-url $RPC` 应返回有代码地址。`[代码已证]`

`staker` endpoint 读回进 readiness check：脚本 `_readAddress(OMNICHAIN_MEMECOIN_STAKER, "localEndpoint()")` 读回值须 `== endpoints[uint32(block.chainid)]`（错误串 `STAKER_ENDPOINT_NOT_READY`）——与 dispatcher 读回对称（`YIELD_DISPATCHER_ENDPOINT_NOT_READY`），拦截 endpoints 配成非零但错误值时 `lzCompose` 恒 `PermissionDenied` 的配置错误。`[代码已证]`

dispatcher endpoint 读回进 readiness check：脚本 `_readAddress(MEMEVERSE_YIELD_DISPATCHER, "localEndpoint()")` 读回值须 `== endpoints[uint32(block.chainid)]`（错误串 `YIELD_DISPATCHER_ENDPOINT_NOT_READY`）——拦截 endpoints 配成非零但错误值时 `lzCompose` 恒 `PermissionDenied` 的配置错误。`[代码已证]`

`lzEndpointRegistry` 读回进 readiness check：脚本 typed decode `getLauncherContracts()` 后断言 `launcherContracts.lzEndpointRegistry == LZ_ENDPOINT_REGISTRY`（错误串 `LAUNCHER_REGISTRY_NOT_READY`，预期值由 `_loadReadinessEnv` 从 `LZ_ENDPOINT_REGISTRY` env 装载）——与其余五个依赖 pin 同型，捕获跨会话/双角色部署下 launcher 烧入值与当日 env 漂移（registry 换 nonce 重部署或 env 抄错）；该 env pin 另经 `_requireContractCode` 校验有代码（错误串 `REGISTRY_CODE_NOT_READY`，该码检位于 `MemeverseScript.s.sol::_requireRegistrationCenterReady`、onboard 路径同样生效），与其余依赖码检同型，拦截「launcher 烧入 codeless 预测地址 + readiness env 同错值」的等值穿透；registry 为 initialize-only（无 setter），错配无法运行时修复、只能重部署，故须在 registration 打开前拦截；受影响运行时路径为 `MemeverseLaunchImpl.sol::_lzConfigure` 远端注册 `lzEndpointIdOfChain`→`setPeer` 与 `MemeverseSettlementImpl.sol::_sendRedeemedFeesCrossChain` 跨链 fee 分发 EID 解析，及 view 报价路径 `MemeverseFeePreviewReader.sol::quoteDistributionLzFee`（读 `contracts.lzEndpointRegistry` 解析 govEndpointId）。覆盖面含 launcher 字段与两个自有 registry 读回：center 侧 `MemeverseScript.s.sol::_requireRegistrationCenterReady` 追加断言：经 `_readAddress(registrationCenter, "lzEndpointRegistry()")` staticcall 读回的值 `== LZ_ENDPOINT_REGISTRY`（错误串 `REGISTRATION_CENTER_REGISTRY_NOT_READY`，与 `DAY()` 读回同型；center 为 proxy 部署（UUPS），`lzEndpointRegistry` 为 `initialize` 写入的 storage 且无 setter，错配运行时不可修复、仍须重部署——fail-closed 结论不变）；interoperation 侧引入新必需 env `MEMEVERSE_OMNICHAIN_INTEROPERATION`（`_loadScriptEnv` / `_loadReadinessEnv` 双装载，经 `_optionalEnvAddress` 装载、与 POLEND/POLSPLITTER 同型：`run()` 部署 `MemeverseOmnichainInteroperation` 后回写实际地址、同 run 的 readiness 用回写值，无需预填；standalone readiness 必须提供该 env，缺失时以 `INTEROPERATION_CODE_NOT_READY` 具名失败；env 值无需等于实际部署地址（回写覆盖）），`_requireDeploymentReady` 对其先 `_requireContractCode(..., "INTEROPERATION_CODE_NOT_READY")` 码检、再断言 `_readAddress(..., "LZ_ENDPOINT_REGISTRY()") == LZ_ENDPOINT_REGISTRY`（错误串 `INTEROPERATION_REGISTRY_NOT_READY`，预期值同源 `LZ_ENDPOINT_REGISTRY` env）；受影响运行时路径相应补 `MemeverseRegistrationCenterUpgradeable.sol::quoteSend` 与 `MemeverseOmnichainInteroperation.sol::memecoinStaking` 治理链在异链时的跨链 OFT send，及 view 报价 `MemeverseOmnichainInteroperation.sol::quoteMemecoinStaking`（治理链在异链时，经共享 `_buildStakingSendParam` 消费同一 immutable）。内容探针把校验面从「registry 地址相等」扩到「registry 内容正确」：脚本 `_requireDeploymentReady` 对 `MemeverseScript.s.sol::_chainsInit` 装载的单源链表（`_chainsInit` 同时吸收 `omnichainIds` 赋值，链表与 `endpointIds` env 装载同源单点）全 3 链逐一断言 `ILzEndpointRegistry(LZ_ENDPOINT_REGISTRY).lzEndpointIdOfChain(chainId) == endpointIds[chainId]`（错误串 `REGISTRY_PAIR_NOT_READY`），并断言本地锚点 `lzEndpointIdOfChain(uint32(block.chainid)) == 本地 endpoint 的 eid() 读回`（错误串 `REGISTRY_LOCAL_EID_NOT_READY`，env 无关的链上真值锚、覆盖本链切片）。三个漂移向量由此覆盖：跨会话 env 分叉（上述各 pin 只验地址相等；registry 内表内容由内容探针另行覆盖）、registry 换 nonce 重部署后旧实例存活（env pin 仍指旧地址、其表内容陈旧）、开闸前 owner 经 `LzEndpointRegistry.sol::setLzEndpointIds` 重指表项。覆盖面 carve-out：内容探针与本地锚点覆盖 `run()` 与 `::openSupportedUAssetsAfterReadiness` 两条路径；`::onboardUAsset` 只调 `_requireRegistrationCenterReady`、不经 `_requireDeploymentReady`，内容探针与本地锚点在该路径不触发（规范流程下首次开闸必经 `::openSupportedUAssetsAfterReadiness`——launcher fundMetaDatas UETH/UUSD 检查强制——探针仍先行校验，仅非规范直走 `::onboardUAsset` 的用法绕过）。认识论边界：单会话 run() 内探针恒真（`MemeverseScript.s.sol::_deployLzEndpointRegistry` 与探针同源读同一 `endpointIds` env，自洽校验，价值在跨会话 / standalone readiness）；开闸后 owner 重指不在闸门射程（运行时无 registry 内容校验）；远端链同源同错（env 全链抄错同一 eid）仍盲，仅本链切片由锚点覆盖。残余观察：registrar / registration center 部署期 `setPeer` 直接消费 `endpointIds` env、不经 registry（`MemeverseScript.s.sol::_deployRegistrationCenter` / `::_deployMemeverseRegistrar`），远端 eid 同源错配任何 registry 探针不覆盖，属部署清单人工核对步骤（见上方逐字段校验区人工核对行）。部署期另有预测-vs-实际断言闸门：`MemeverseScript.s.sol::_deployLzEndpointRegistry` 在 CREATE3 部署后、`setLzEndpointIds` 写入 pairs 前 `require` 实际部署地址 == `LZ_ENDPOINT_REGISTRY` env pin（错误串 `REGISTRY_DEPLOY_MISMATCH`，先例 `YIELD_DISPATCHER_PROXY_MISMATCH` 同型）；操作前提：执行该部署步骤前，先经 `MemeverseScript.s.sol::_getDeployedLzEndpointRegistry(nonce)` 取当前 nonce 的 CREATE3 预测地址预填该 env；失败形态：forge 全量模拟在 broadcast 前中止整个 run——无链上孤儿实例、无部分 pairs 写入。`[代码已证]`
...
独立入口 `openSupportedUAssetsAfterReadiness` 与 `run()` 流程一致：内部先 `_loadReadinessEnv()` 装载 `OMNICHAIN_MEMECOIN_STAKER`、`LZ_ENDPOINT_REGISTRY` 与 `MEMEVERSE_OMNICHAIN_INTEROPERATION`，再 `_chainsInit()` 装载全部 3 链 ENDPOINT/EID env；任一 env 缺失时该入口响亮 revert，而非静默跳过检查。`[代码已证]`

registration center `DAY` 读回进 readiness check：`MemeverseScript.s.sol::_requireRegistrationCenterReady`（由 `::_openSupportedUAssetsAfterReadiness` 与 `::onboardUAsset` 两条 registration 打开路径调用）断言 `IMemeverseRegistrationCenter(registrationCenter).DAY() == expectedRegistrationDay`，前置 `_requireContractCode(registrationCenter, "REGISTRATION_CENTER_CODE_NOT_READY")`（脚本存储变量，`_loadReadinessEnv` 从 `EXPECTED_DAY` env 装载：testnet 部署在 `.env` 设 `EXPECTED_DAY=180` 保留快窗，未设默认生产值 `24 * 3600`；错误串 `REGISTRATION_DAY_NOT_READY`）——`DAY` 为源码级 `constant`，错误值烧入 implementation 字节码、无 setter 可救；该断言把「测试值漏改生产值」从人工记忆转为部署闸门，fail-closed 朝向生产值（mainnet 漏改 180 会被阻断），任一路径失配即阻断 registration 打开。`[代码已证]`

依赖 launcher back-reference 进 readiness check：脚本 `_requireDeploymentReady` 断言 registrar / proxyDeployer / yieldDispatcher 三个依赖的反向指针各自指回 Launcher proxy——`_readAddress(MEMEVERSE_REGISTRAR, "MEMEVERSE_LAUNCHER()") == MEMEVERSE_LAUNCHER`（错误串 `REGISTRAR_LAUNCHER_NOT_READY`）、`_readAddress(MEMEVERSE_PROXY_DEPLOYER, "memeverseLauncher()") == MEMEVERSE_LAUNCHER`（错误串 `PROXY_DEPLOYER_LAUNCHER_NOT_READY`）、`_readAddress(MEMEVERSE_YIELD_DISPATCHER, "memeverseLauncher()") == MEMEVERSE_LAUNCHER`（错误串 `YIELD_DISPATCHER_LAUNCHER_NOT_READY`），与 `POLEND_LAUNCHER_NOT_READY` 读回同型——在单向 pin（launcher 烧入依赖地址）之外补依赖侧对角校验。`[代码已证]`

launcher `localLzEndpoint` pin：脚本 `MemeverseScript.s.sol::_checkMemeverseLauncherDeployment` 断言 `getLauncherContracts().localLzEndpoint == localEndpoint`（`endpoints[uint32(block.chainid)]` 装载的本链 endpoint 地址，错误串 `LAUNCHER_ENDPOINT_MISMATCH`，与同函数 `LAUNCHER_OWNER_MISMATCH` / `LAUNCHER_POLEND_MISMATCH` / `LAUNCHER_SPLITTER_MISMATCH` 同型）——launcher 烧入的本链 endpoint 与部署时 endpoint env 的漂移在部署核验即被拦截。`[代码已证]`
...
# swap-ready 检查（脚本 _requireSwapReady 子检查全集，facet 检查未覆盖的不变量）:
# - hook flags: hook 的 flags 读回 == MEMEVERSE_HOOK_FLAGS（错误串 HOOK_FLAGS_NOT_READY）
# - launcher.memeverseSwapRouter == swapRouter（错误串 LAUNCHER_ROUTER_NOT_READY）
# - launcher.memeverseUniswapHook == hook（错误串 LAUNCHER_HOOK_NOT_READY）
# - router.hook == hook（错误串 ROUTER_HOOK_NOT_READY）
# - hook.launcher == launcher（错误串 HOOK_LAUNCHER_NOT_READY）
# - hook.poolInitializer == router（错误串 HOOK_POOL_INITIALIZER_NOT_READY）
# - router 与 hook 的 poolManager 对角一致（错误串 ROUTER_POOL_MANAGER_NOT_READY）
# lpTokenImplementation readiness 见 §3.9.5 与 §3.10 codehash 表
cast call $SWAP_ROUTER "poolManager()(address)" --rpc-url $RPC
# 必须等于 $HOOK_PROXY 的 poolManager() 读回；不一致时全部 swap/池初始化 NotPoolManager revert
# （错误串 ROUTER_POOL_MANAGER_NOT_READY）
```
...
cast call $POLEND_PROXY "treasury()(address)" --rpc-url $RPC
# 应等于 POLEND_TREASURY 预期值
```
...
cast call $SPLITTER_PROXY "polend()(address)" --rpc-url $RPC
# 应等于 POLendUpgradeable proxy 地址（back-reference）
```
...
**per-uAsset 配置 readiness `[代码已证]`：**

每个支持的 `uAsset` 须有非零 `fundMetaDatas(uAsset).minTotalFund` 与 `fundBasedAmount`，且派生虚拟资产缓冲 `virtualAssets = minTotalFund × fundBasedAmount × 7 / 1000 > 0`（等价 `minTotalFund × fundBasedAmount >= 143`）；且 `POLendUpgradeable.settlementDustStates(uAsset).maxReserve > 0`。任一为空/为零会让该 `uAsset` 的建池/结算路径在打开后异常，须在 readiness 阶段逐 `uAsset` 校验。
...
6. Hook proxy（`ERC1967Proxy` + UUPS，initialize 绑定 owner/treasury/lpTokenImplementation/3 facet 地址/launcher）
...
**部署顺序约束**。Hook proxy 在部署时立即以 3 facet 地址与配置初始化。hook 的 `initialize()` 会校验 facet 地址非零，并经 `_requireFacetPoolManager` 校验 facet 字节码就绪（`FacetCodeNotReady`）且 poolManager 与 hook 一致（`FacetPoolManagerMismatch`、getter 不可读时 `FacetPoolManagerUnreadable`），因此必须先完成 LP token implementation、3 个 facet 与 Hook implementation 的顺序部署，最后部署 Hook proxy。Hook proxy 地址在部署前已通过 CREATE3 salt 挖矿确定。
...
- 返佣记账为纯 storage effect，先于 treasury take 与 rebate take，最后 emit（严格 CEI）；全部调用点位于 PoolManager unlock session 内，`SwapFacet` logic 函数开头检查 `address(this) != __self` 防直接 CALL。CEI 时序、守恒与调用面唯一权威见 [docs/spec/swap/uniswap-v4.md §3.3](spec/swap/uniswap-v4.md) 与 [INV-20](spec/invariants.md)。`[代码已证]`
...
- 影响面（全挡，pause 期间全部 revert `EnforcedPause`）：用户 token 移动、claim、burn、OFT 桥接，以及协议 credit 路径（credit 抵扣、Refund credit 退回、finalize 托管 burn）全部阻断；real-only 分支不受影响的分支细节见 [docs/spec/polend/genesis.md §4.1](spec/polend/genesis.md)。incident 期间须权衡对 verse 生命周期编排的影响。
- OFT 提示：home pause 期间 LayerZero 消息 delivery 失败转 retryable/storeable、不丢失，unpause 后经 LayerZero 重试落地；每链 pause 状态独立，须逐链核对 `paused()`。
...
入口：`MemecoinYieldVault.reAccumulateYields(dispatcher, guid, message)`（MEMECOIN 路径）或 `YieldDispatcherUpgradeable.settlePendingCompose(token, guid, message)`（MEMECOIN/UASSET 通用）。本节（dispatcher 侧）两个入口都 permissionless——任何地址可执行，不需要 admin 权限；协议有意不提供 onlyOwner 恢复入口。跨链 staking compose 的恢复入口在 `OmnichainMemecoinStakerUpgradeable`，权限与结算语义不同（仅接收人可调、恒释放裸币不建仓），见 §3.13.1。

适用时机（失败都来自外部瞬时原因，不是 vault/governor 损坏——vault/governor 部署即验证，不假设永久故障）：

恢复前提是 payload 金额非零、格式合法——格式合法 = 可解码（内层 ≥ 64 字节、静态元组前两 word 内容有效；尾部忽略，>64 字节可解析帧可经 settle 恢复，见下）。零金额（amountLD=0）与格式非法 payload 均不可经 settle 重试恢复，但 `lzCompose` 路径对这两类都能收敛——完整收敛矩阵见本节末尾「边界——零金额/格式非法 payload 均不可经 settle 重试恢复」条目。
...
- 边界——零金额/格式非法 payload 均不可经 settle 重试恢复（超长帧内容可解析类除外，见下）：
  ...
  - 零金额类（amountLD=0）：`settlePendingCompose` 两条路径仍以 `ZeroInput` 拒绝零金额（YieldDispatcherUpgradeable.sol::settlePendingCompose、OmnichainMemecoinStakerUpgradeable.sol::settlePendingCompose）、不可经 settle 收敛；但 `lzCompose` 路径对除自引用分支外的全部分支收敛（零金额自引用帧被 `lzCompose` 的自引用守卫先于 `_settle` 拦截——守卫无金额检查——emit `ComposeRejected(guid, token, 0)` 而非 `OFTProcessed`，见下条自引用类）——`_settle` 在 amount==0 时短路（不 burn、不记账），槽位→`Settled` + emit `OFTProcessed(amount=0, burnedAtDispatcher=false)`、endpoint 状态机收敛到 `ComposeDelivered`（与 vault 分支的零金额收敛行为一致）
...
#### 3.13.1 跨链 staking compose 的人工重试（OmnichainMemecoinStakerUpgradeable）

入口：`OmnichainMemecoinStakerUpgradeable.settlePendingCompose(memecoin, guid, message)`。权限与 §3.13 的 dispatcher 入口相反：**仅 message 编码的接收人（receiver）本人可调**，其他地址（含运维）调用 revert `NotBeneficiary`——运维不能代办，职责是识别滞留后通知接收人本人执行；协议同样不提供 onlyOwner 恢复入口。

部署期守卫：interoperation 构造器仍拒绝零地址（`ZeroAddress()`）；staker 构造器仅 `_disableInitializers()`，零值检查在 `initialize`（对 `initialOwner` / `_localEndpoint` 具名零检查，镜像 dispatcher 模式）。部署脚本对 `localEndpoint`/`MEMEVERSE_LAUNCHER`/`OMNICHAIN_MEMECOIN_STAKER`/`OUTRUN_DEPLOYER` 零配置以具名错误在部署期失败（`ZERO_LOCAL_ENDPOINT` / `ZERO_MEMEVERSE_LAUNCHER` / `ZERO_OUTRUN_DEPLOYER` / `ZERO_OMNICHAIN_MEMECOIN_STAKER`）——`ZERO_OUTRUN_DEPLOYER` 覆盖由 dispatcher 扩展至 interoperation 与 staker 部署函数（`_deployMemeverseOmnichainInteroperation` / `_deployOmnichainMemecoinStaker` 亦前置 require `OUTRUN_DEPLOYER != address(0)`）；错误配置不再静默产出不可用 composer；恢复入口语义（§3.13/§3.13.1 所述）不变。`[代码已证]`
...
最小推荐值以下表实测为基准。测量由 `test/interoperation/LzComposeGasBenchmark.t.sol`（`gasleft()` 前后差，仅计 `lzCompose` 主体、不计 endpoint wrapper）给出，采用**暖状态模型**：基准测试在测量前对 vault/governor 做一次预结算以把生产中长期暖的存储槽（`totalAssets`、checkpoint、share 余额、allowance、dispatcher 托管余额）预热至 EIP-2929 暖价，被测 guid 的 compose-state 槽保持新建（与生产每个 guid 仅 resolve 一次一致）。直接对全新 clone vault 测量会因冷槽附加费高估约 3.1×（staker deposit 分支冷测 ~268k vs 暖测 ~86k（当前实现读数），268/86≈3.1；冷测读数未随当前实现重测），故必须按暖状态读数。

| composer / 分支 | 暖状态实测主体 gas | 脚本默认预算 | 判定 |
|---|---|---|---|
| staker deposit（vault 有 code：`asset()` + `_safeApprove`×2 + `deposit`）| ~86000（实测 86,082，含 ERC1967Proxy delegatecall hop 与 ERC-7201 storage 读开销）| `omnichainStakingGasLimit`=135000（`MemeverseOmnichainInteroperation` 构造器第 6 参，`script/MemeverseScript.s.sol`）| 足够（余量 ~49k 覆盖 endpoint wrapper + 冷访问尾） |
| staker fallback（vault 无 code：`_transferOut` 直转）| ~70600 | 135000 | 足够 |
...
- 兜底结算（compose 失败人工重试，§3.13）：`ComposeSettled`（YieldDispatcherUpgradeable）/ `StakingComposeSettled`（OmnichainMemecoinStakerUpgradeable）
- 畸形 compose 消费：`ComposeRejected(guid, token, amount)`——不可结算的 compose payload 被消费（置 `Settled`、不结算）的信号，用于监控发送端畸形/自伤消息与对账。两个 composer 同签名发出：`YieldDispatcherUpgradeable`（畸形内层 composeMsg 解析失败，或干净可解析帧 receiver == dispatcher 自身地址的自引用帧——见 §3.13 自引用类）与 `OmnichainMemecoinStakerUpgradeable`（64 字节帧 receiver word 高位脏）。receiver 非自引用的内容可解析 >64 帧不触发 ComposeRejected：dispatcher 正常结算（发正向/兜底事件），staker `lzCompose` 长度守卫 revert `MalformedComposeMsg`（无事件、槽 None）。
...
- 行业对齐：项目直接采用 OpenZeppelin v5.5+ 的 `ReentrancyGuardTransient`（`ReentrancyGuardTransient.sol`）作为统一重入保护；OZ 同样声明 "This variant only works on networks where EIP-1153 is available"。`[代码已证]`
