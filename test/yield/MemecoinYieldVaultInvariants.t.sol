...
///      let the fuzzer substitute arbitrary observers and break INV-V7 tracking).
...
contract YieldVaultHandler is Test {
    ...
    /// @dev INV-V7: a fixed holder's share value never decreases across any handler op.
    ...
    function _assertSentinelNotDecreased(uint256 preValue) internal {
        assertGe(_shareValue(sentinel), preValue, "INV-V7: sentinel share value decreased");
    }
    ...
    /// @dev Foreign-op value-growth accounting for INV-V5: a deposit/mint/requestRedeem
    ...
    /// INV-V1/V3: deposit side (floor rounding).
    function deposit(uint256 actorSeed, uint256 amountSeed) external {
        ...
        try vault.deposit(amount, who) returns (uint256 shares) {
            ...
            // INV-V3: a truly empty vault (S == 0 AND T == 0) mints exactly 1:1. A
            ...
            if (supplyBefore == 0 && assetsBefore == 0) {
                assertEq(shares, amount, "INV-V3: empty-vault deposit not 1:1");
            } else {
                assertGt(shares, 0, "INV-V3: zero-share deposit");
            }
        } catch {}
        vm.stopPrank();
    ...
    /// INV-V1/V6: mint side (ceil rounding); actor funded with the previewed cost.
    ...
    /// INV-V2/V5: burn shares into the redemption queue (floor lock pricing).
    function requestRedeem(uint256 actorSeed, uint256 sharesSeed) external {
        ...
        try vault.requestRedeem(shares, who, who) returns (uint256 locked) {
            // INV-V2 premise recorded at push time: rate >= 1 keeps locked >= shares,
            // which keeps per-entry floor payouts non-zero (lockstep decrements preserve it).
            assertGe(locked, shares, "INV-V2: locked below shares at push");
    ...
    /// INV-V2/V8: shares-first claim of matured entries.
    ...
    /// INV-V6/V8: assets-first claim; InsufficientClaimableRedeem is an expected floor
    ...
    /// INV-V4/V5: yield push — burned while the vault is empty, credited otherwise.
    function accumulateYields(uint256 yieldSeed) external {
        ...
        try vault.accumulateYields(amount) {
            // INV-V5: exact per-holder value growth from the credited yield. An empty
    ...
    /// INV-V4-2: a direct ERC20 donation must not move accounting-based pricing. Both
...
/// @notice Stateful and single-sequence properties for MemecoinYieldVault (audit batch
///         2026-08-19, design doc §2, INV-V1..V8 minimal incremental set).
...
contract MemecoinYieldVaultInvariants is StdInvariant, Test {
    ...
    // Fixed holder that never transacts — observes INV-V7 value monotonicity.
    address internal sentinel;

    uint256 internal constant VIRTUAL_ASSETS = 1e24; // production-scale buffer keeps INV-V5 meaningful
    ...
    /// INV-V1: totalSupply() <= totalAssets() in every reachable state (rate >= 1 is
    /// the premise for INV-V2's lockstep and redeem's non-zero payout guard).
    function invariant_supplyNeverExceedsAssets() external view {
        assertLe(vault.totalSupply(), vault.totalAssets(), "INV-V1");
    }

    /// INV-V4-2: the frozen before/after snapshots around a direct ERC20 donation are
    ...
    function invariant_directDonationDoesNotChangeRate() external view {
        assertEq(
            ...
            "INV-V4-2: donation moved the rate"
        );
    }

    /// INV-V5: no actor receives more than paid in plus their exactly-measured
    ...
    function invariant_noActorRedeemsMoreThanPaidPlusObservedGrowth() external view {
        for (uint256 i; i < 3; ++i) {
            ...
            assertLe(
                ...
                "INV-V5: payout above damped bound"
            );
        }
    }

    /// INV-V8-1: solvency — holdings cover accounting plus every queued lock. The
    ...
    function invariant_holdingsCoverAccountingPlusQueue() external view {
        ...
        assertGe(
            ...
            "INV-V8-1: holdings below accounting plus queue"
        );
    }

    // ---- standalone single-sequence fuzz properties ----

    /// INV-V4-1: yield pushed to an empty vault is burned, never credited — the
    ...
    function testFuzz_EmptyVaultYieldBurnedNotCredited(uint96 y) external {
        ...
        vm.stopPrank();
        assertEq(v.totalAssets(), 0, "INV-V4-1: empty vault credited yield");
        assertEq(v.totalSupply(), 0);
        assertEq(asset.balanceOf(address(v)), 0, "INV-V4-1: yield not burned");
    }

    /// INV-V3: on a fresh vault, deposit is exactly 1:1 and chained mints stay exactly
    ...
    /// INV-V5: single-attacker donation profit bound. After deposit(d) -> yield(y),
    ...
    /// INV-V6 (incremental half — previewDeposit floor already covered elsewhere):
