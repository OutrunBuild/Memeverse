...
| `claimRefund` | 用户 | market 为 `Refund` | `to != address(0)`，有未领取利息 | 标记 `refundClaimed`；GenesisCredit pause 期间 credit 部分被阻断（real-only 不受影响），见 [genesis.md §4.1](genesis.md) / [INV-27](../invariants.md) |
