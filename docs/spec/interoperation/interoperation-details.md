...
这里的 `retry`（重试）是重新执行尚未成功的目标链处理，不是重新发起一次源链 send。源链 OFT send 成功后，目标链 `lzReceive` / `lzCompose` 失败不会回滚源链状态。对未执行的 memecoin yield compose，`MemecoinYieldVault.reAccumulateYields(dispatcher, guid, message)` 是 permissionless 恢复入口：内部委托 `IYieldDispatcher(dispatcher).settlePendingCompose` 完成全部结算（settle 在 approve 前做 token↔vault 的 `asset()` 绑定校验，空 vault 按既有规则 burn；零金额 payload 在 settle 入口以 `ZeroInput` 拒绝）。

关键边界：`dispatcher` 必须取 endpoint `ComposeSent` 事件的 `to` 字段（vault 不存储 dispatcher，launcher 旋转后可能是历史 dispatcher）；`message` 从同一事件原样拷贝；入口按序校验 message 长度（<108 字节 revert `ComposeMessageTooShort`）、内层 receiver 为本 vault（`NotComposeBeneficiary`）与 settle 返回非零（`ComposeSettlementFailed`）。完整恢复步骤、错传 dispatcher/staker 的失败矩阵与畸形帧边界唯一权威见 [operations.md §3.13](../../operations.md)；`verifySettle` 投递证明机制见 [layerzero-oapp-oft.md §4](layerzero-oapp-oft.md)。

该入口只处理仍未结算（`ComposeState.None`）的 compose transfer；它是 destination accumulation 的 retry path，不会撤销已成功的 source send。
