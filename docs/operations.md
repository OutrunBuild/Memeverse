...
center（`MemeverseRegistrationCenterUpgradeable`）的 registrar 是 owner 可变的 storage 指针（`MemeverseRegistrationCenterUpgradeable.sol::setMemeverseRegistrar`，onlyOwner、零值具名拒绝、emit `SetMemeverseRegistrar(old, new)`），不再是构造期烧入的 immutable。更换 registrar（registrar 合约损坏/密钥事件后的 deadlock 解锁）时必须成对操作：
...
独立入口 `openSupportedUAssetsAfterReadiness` 与 `run()` 流程一致：内部先 `_loadReadinessEnv()` 装载 `OMNICHAIN_MEMECOIN_STAKER`、`LZ_ENDPOINT_REGISTRY` 与 `MEMEVERSE_OMNICHAIN_INTEROPERATION`，再 `_chainsInit()` 装载全部 9 链 ENDPOINT/EID env；任一 env 缺失时该入口响亮 revert，而非静默跳过检查（此前只 `_loadReadinessEnv` 不填 endpoints 映射、恒误拒绝）。`[代码已证]`

registration center `DAY` 读回进 readiness check：`MemeverseScript.s.sol::_requireRegistrationCenterReady`（由 `::_openSupportedUAssetsAfterReadiness` 与 `::onboardUAsset` 两条 registration 打开路径调用）断言 `IMemeverseRegistrationCenter(registrationCenter).DAY() == expectedRegistrationDay`，前置 `_requireContractCode(registrationCenter, "REGISTRATION_CENTER_CODE_NOT_READY")`（脚本存储变量，`_loadReadinessEnv` 从 `EXPECTED_DAY` env 装载：testnet 部署在 `.env` 设 `EXPECTED_DAY=180` 保留快窗，未设默认生产值 `24 * 3600`；错误串 `REGISTRATION_DAY_NOT_READY`）——`DAY` 为源码级 `constant`（center 改 UUPS 后保留不动），错误值烧入 implementation 字节码、无 setter 可救；该断言把「测试值漏改生产值」从人工记忆转为部署闸门，fail-closed 朝向生产值（mainnet 漏改 180 会被阻断），任一路径失配即阻断 registration 打开。`[代码已证]`
...
> 误用警告（dispatcher 地址来源）：vault 已移除 `yieldDispatcher` storage 槽（getter 不存在），恢复用的 `dispatcher` **必须**取自步骤 1 的 `ComposeSent` 事件 `to` 字段；launcher 的 `setYieldDispatcher` 旋转后 compose 可能滞留在历史 dispatcher；按错误地址调用会进错 dispatcher 的空 `composeQueue` 槽 → revert `NotDelivered`（见步骤 3 失败矩阵）。`[代码已证]`
