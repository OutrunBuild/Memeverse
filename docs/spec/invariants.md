...
- 约束（原子生命周期）：`beginAccountSession() -> 单一经济账户 Router -> endAccountSession()` 必须在同一不可捕获、全成全败的执行 frame 内完成。显式 `end` 必须清除 Hook transient session context，不能依赖交易结束自动清理，也不能省略；失败路径不得留下可被后续 callback 使用的 context。
